...
def _run_inference_batch(config, model, foveator, dataset, device: torch.device, count: int) -> None:
    for index in range(count):
        ...
        _ = pred_masks.squeeze(0)[pred_iou.squeeze(0).argmax()]
...
def main() -> None:
    ...
    _run_inference_batch(config, model, foveator, measured_dataset, device, args.max_examples)
    torch.cuda.synchronize(device)
