...
def _resolve_default_annotations(root: Path, split: str) -> Path:
    candidates = [
        ...
        root / "data" / "annotations" / f"{split}.json",
        root / "data" / "annotations" / f"instances_{split}.json",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise FileNotFoundError(f"Could not find ZeroWaste-f annotations for split '{split}' under {root}")
...
def _resolve_default_images_dir(root: Path, split: str) -> Path:
    ...
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise FileNotFoundError(f"Could not find ZeroWaste-f images for split '{split}' under {root}")


def main() -> None:
    parser = argparse.ArgumentParser("Create an STT eval manifest for ZeroWaste-f.")
